...
export type MisionSectionPayload = {
    id: number;
    descripcion?: unknown;
    titulo?: unknown;
    attributes?: {
...
export async function getMisionSection(): Promise<MisionSectionPayload[]> {
    try {
        const respuesta = (await query("/mision?locale=es-CL")) as StrapiCollectionResponse;
        ...
        return elementos
            .map((registroBruto) => {
                ...
                } satisfies MisionSectionPayload;
            })
            .filter((registro) => registro.id > 0 && Boolean(registro.descripcion || registro.titulo));
    } catch (errorCapturado) {
        console.error("Error obteniendo sección misión:", errorCapturado);
        return [];
    }
}
